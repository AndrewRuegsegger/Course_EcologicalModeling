...
unlink("C:/Users/Andrew/AppData/Local/R/win-library/4.4/00LOCK-rlang", recursive = TRUE)
...
# Other posterior central tendencies
b_year <- as_draws(fit_brm,
                   "b_Year_scaled")
b_year_chain1 <- b_year$`1`
hist(unlist(b_year_chain1))
credInt_90 <- data.frame(
  CI_0.05 = quantile(unlist(b_year_chain1), 0.05),
  CI_0.95 = quantile(unlist(b_year_chain1), 0.95)
  #median = summarize(unlist(b_year_chain1), 0.50)
)
credInt_90
...
pp_check(fit_brm)
# yrep is distribution of what the model outputs; y = response distribution
# you want the two lines to match up as much as possible
# this model is predicting negative whistle counts b/c using a normal distr

# Look at traceplots for each parameter estimate
plot(fit_brm)
# don't wanna see snakes in traceplots = indicating autocorrelation
# you wanna see hairy caterpillars
# these are good 
...
# negative estimates; also estimates are not super smooth
...
pp_check(fitTrunc0_brm) # better fit
pp_check(fit_brm)
...
sum(whistle$WhistleCount == 0, na.rm = TRUE)

# one way to get around the problem of there being zeroes and using 
# distributions that can't take zeroes; force a small baseline
glm(WhistleCount + 0.00001 ~ 1,
    family = Gamma,
    data = whistle)

# another way: hurdle models =
# there can be actual zeroes, but you need to get over this hurdle to get to 
# other numbers.
